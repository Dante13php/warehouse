...
- The middleware selects one of two **mutually exclusive** strategies by the `User-Agent` header (hard channel split, no precedence, no fallback): a browser-signature `User-Agent` ⇒ **JWT bearer** channel only (UI/refresh-token sessions); a non-browser or missing/empty `User-Agent` ⇒ **API key** channel only (external API clients, via `ApiKeyStorage` resolving the configured `api_key_header`). The credential type for the non-selected channel is never consulted (not read, not decoded, not an error). The browser-signature token list is the in-module constant `_BROWSER_UA_SIGNATURES` in `auth_middleware.py`.
- `User-Agent` selects only *which channel runs*; it never grants identity by itself, and both channels still require a valid credential.
